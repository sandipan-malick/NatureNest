...
export default function ProductCart() {
  ...
  if (loading)
    return <p className="mt-20 text-center text-gray-600 animate-pulse">Loading cart...</p>;
  if (error) return <p className="mt-20 text-center text-red-500">{error}</p>;

  return (
    <div className="max-w-6xl min-h-screen p-6 mx-auto bg-green-50">
      <h1 className="mb-8 text-4xl font-bold text-center text-green-700">Your Cart</h1>

      {/* Navigation */}
      <div className="justify-start hidden gap-4 mb-6 md:flex">
        <Link className="flex items-center gap-2 px-4 py-2 text-white bg-green-600 rounded-lg hover:bg-green-700" to="/">
          <FaHome /> Home
        </Link>
        <Link className="flex items-center gap-2 px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700" to="/all-product">
          <FaBox /> Products
        </Link>
      </div>
      ...
      <div className="p-4 mb-6 bg-white rounded-lg shadow-md">
        ...
        {addresses.length === 0 ? (
          <div className="flex flex-col items-start gap-2">
            ...
            <Link to="/address" className="px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700">
              Add Address
            </Link>
          </div>
        ) : (
        ...
        <>
            ...
            {cartItems.map((item) => (
              <div key={item._id} className="flex flex-col md:flex-row items-center justify-between p-4 bg-white rounded-lg shadow-md hover:shadow-xl transition transform hover:scale-[1.02]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="font-semibold text-green-900">{item.productName}</h2>
                ...
                <button
                  ...
                  className="flex items-center px-3 py-1 mt-4 text-white transition bg-red-600 rounded md:mt-0 hover:bg-red-700"
                >
          ...
          <div className="flex flex-col items-center justify-between p-4 mt-8 space-y-4 rounded-lg shadow-md md:flex-row bg-green-50 md:space-y-0 md:space-x-4">
            ...
            <button
              ...
              className="px-8 py-3 text-white transition bg-blue-600 rounded-lg disabled:opacity-50 hover:bg-blue-700"
            >
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-green-800 shadow-inner md:hidden">
        <div className="flex justify-around py-2">
          <Link to="/" className="flex flex-col items-center text-white hover:text-green-300">
            <FaHome size={18} /><span className="text-xs">Home</span>
          </Link>
          <Link to="/all-product" className="flex flex-col items-center text-white hover:text-blue-300">
            <FaBox size={18} /><span className="text-xs">Products</span>
          </Link>
        </div>
      </footer>
    </div>
