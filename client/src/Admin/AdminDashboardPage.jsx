...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const res = await axios.get("https://naturenest-y4n0.onrender.com/admin-dashboard", {
  ...
  const handleLogout = async () => {
    try {
      await axios.post(
        "https://naturenest-y4n0.onrender.com/api/admin/logout",
        {},
        { withCredentials: true }
      );
      navigate("/admin-login");
        ...
        {admin ? (
          <div className="p-6 bg-white rounded-lg shadow-md">
            ...
            <p className="text-gray-600">
              Use the navigation menu to manage product and order.
            </p>
          </div>
        ) : (
